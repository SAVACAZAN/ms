<template>
  <b-tabs content-class="mt-3" v-model="activeTab">
    ...
    <b-tab title="Open Orders">
      ...
      <b-table :items="openOrders.data" :fields="openOrders.fields" sort-icon-left small sticky-header="true"
        no-border-collapse responsive>
    ...
    <b-tab title="Closed Orders">
      <b-table :items="closedOrders.data" :fields="closedOrders.fields" sort-icon-left small sticky-header="true"
        no-border-collapse responsive>
      </b-table>
    </b-tab>

    <b-tab title="Grid Orders" active>
      <b-table :items="gridOrders.data" :fields="gridOrders.fields" sort-icon-left small>
                ...
                <tbody>
                  ...
                  <tr v-for="(order, index) in row.item.orders" :key="order.id">
                    ...
                    <ul class="order-details">
                      ...
                      <li :class="{ 'text-success': order.side === 'buy', 'text-danger': order.side === 'sell' }">
                      ...
                      <li :class="{ 'text-success': order.side === 'buy', 'text-danger': order.side === 'sell' }">
                        <strong>gridtotalUSD:</strong> {{ order.amount }}
                      </li>
    ...
    <li :class="{ 'text-success': order.side === 'buy', 'text-danger': order.side === 'sell' }">
                        <strong>newPriceSell:</strong> {{ order.newPriceSell }}
                      </li>
                      <li :class="{ 'text-success': order.side === 'buy', 'text-danger': order.side === 'sell' }">
                        <strong>newAmountSell:</strong> {{ order.newAmountSell }}
                      </li>
                 
                 

                      <li :class="{ 'text-success': order.side === 'buy', 'text-danger': order.side === 'sell' }">
                        <strong>gridtotal:</strong> {{ order.gridtotal }}
                      </li>



                      <li :class="{ 'text-success': order.side === 'buy', 'text-danger': order.side === 'sell' }">
                        <strong>deviationPRICEBuy:</strong> {{ order.deviationPRICEBuy }}
                      </li>
                      <li :class="{ 'text-success': order.side === 'buy', 'text-danger': order.side === 'sell' }">
                        <strong>deviationAMOUNTBuy:</strong> {{ order.deviationAMOUNTBuy }}
                      </li>

                      <li :class="{ 'text-success': order.side === 'buy', 'text-danger': order.side === 'sell' }">
                        <strong>deviationPRICESell:</strong> {{ order.deviationPRICESell }}
                      </li>
                      <li :class="{ 'text-success': order.side === 'buy', 'text-danger': order.side === 'sell' }">
                        <strong>deviationAMOUNTSell:</strong> {{ order.deviationAMOUNTSell }}
                      </li>
    ...
    <b-tab title="DCA Bots">
      <b-table :items="dcaBots.data" :fields="dcaBots.fields" sort-icon-left small>
        ...
        <template #row-details="row">
          <b-card>
            <b-table :items="row.item.deals" :fields="deals.fields" small>
