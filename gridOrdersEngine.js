  ...
  start: async function() {
      ...
      for (const grid of runningGrids) {
        ...
        for (let index = 0; index < grid.orders.length; index++) {
          ...
          if(orderResponse.success) {
            ...
            let amount = orderResponse.order['info']['amount'];
            let newSide = false;

            if(status === 'done' || status === 'closed' || status === 'FAILURE' || status === 'FILLED') {
             ...
             if (newSide === 'buy') {
              ...
              console.log(`Profit \x1b[32mfor 📗 ${newSide}: ${difBUY * newAmount}\x1b[37m`);
            } else if (newSide === 'sell') {
              totalProfitSell += difSELL * newAmount; // actualizam profitul total pentru sell
              console.log(`Profit \x1B[31for 📕${newSide}: ${difSELL * newAmount}\x1b[37m`);
            }
              ...
              console.log(`✒️୧ ‧₊˚Tota୧ ‧₊˚profit for \x1b[32mbuy🟢: ${totalProfitBuy}\x1b[37m`);
                 ...
                 console.log(`✒️୧ ‧₊˚Tota୧ ‧₊˚profit for \x1B[31msell🔴: ${totalProfitSell}\x1b[37m`);
                   ...
                   await GridOrdersModel.updateOne({ _id:grid._id }, { orders:newOrders });
              } else {
    ...
    setInterval(() => {
      ...
      if (count === 1 || count % 10 === 1) {
        ...
        console.log(`\x1b[31m${now.toISOString().slice(0, 10)} \x1b[33m${hours}:\x1b[33m${minutes}:${seconds}\x1b[0m`);
      }
